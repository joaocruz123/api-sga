<?php namespace Tests\APIs;

use Illuminate\Foundation\Testing\WithoutMiddleware;
use Illuminate\Foundation\Testing\DatabaseTransactions;
use Tests\TestCase;
use Tests\ApiTestTrait;
use App\Models\Membros;

class MembrosApiTest extends TestCase
{
    use ApiTestTrait, WithoutMiddleware, DatabaseTransactions;

    /**
     * @test
     */
    public function test_create_membros()
    {
        $membros = factory(Membros::class)->make()->toArray();

        $this->response = $this->json(
            'POST',
            '/api/membros', $membros
        );

        $this->assertApiResponse($membros);
    }

    /**
     * @test
     */
    public function test_read_membros()
    {
        $membros = factory(Membros::class)->create();

        $this->response = $this->json(
            'GET',
            '/api/membros/'.$membros->id
        );

        $this->assertApiResponse($membros->toArray());
    }

    /**
     * @test
     */
    public function test_update_membros()
    {
        $membros = factory(Membros::class)->create();
        $editedMembros = factory(Membros::class)->make()->toArray();

        $this->response = $this->json(
            'PUT',
            '/api/membros/'.$membros->id,
            $editedMembros
        );

        $this->assertApiResponse($editedMembros);
    }

    /**
     * @test
     */
    public function test_delete_membros()
    {
        $membros = factory(Membros::class)->create();

        $this->response = $this->json(
            'DELETE',
             '/api/membros/'.$membros->id
         );

        $this->assertApiSuccess();
        $this->response = $this->json(
            'GET',
            '/api/membros/'.$membros->id
        );

        $this->response->assertStatus(404);
    }
}
